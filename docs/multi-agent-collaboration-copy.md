...
EvidenceLab 把一轮研究输入拆成有限步协作：先装载项目上下文与记忆，再路由意图，随后按需检索证据、审查冲突观点，最后生成决策草稿或资料问答回复。学生/研究者端看到的是连贯对话或任务进度，后台保留检查点与轨迹。
...
历史仓库曾以校园支持场景命名（MindBridge）。本阶段产品语义已切换为科研证据工作区 EvidenceLab；Java 包名暂未迁移，设计文档中的 MindBridge 字样仅用于迁移说明。
